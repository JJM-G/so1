...
// Funkcja zapisuj¹ca wynik CRC na koñcu pliku wejœciowego
int zapiszCRCNaKoncuPliku(char *nazwaPlikuIn, unsigned char reszta[]) {
    // Otwieranie pliku wejœciowego w trybie do³¹czania
    FILE *wskaznikPlikuIn = fopen(nazwaPlikuIn, "ab");
    if (wskaznikPlikuIn == NULL) {
        printf("Nie mozna otworzyc pliku do dopisania CRC: %s\n", nazwaPlikuIn);
        return 1;
    }

    // Zapisywanie wyniku CRC na koñcu pliku
    int i = 0;
    for ( i; i < CRCsize; i++) {
        fwrite(&reszta[i], sizeof(unsigned char), 1, wskaznikPlikuIn);
    }

    fclose(wskaznikPlikuIn);

    printf("Wynik CRC dopisano na koniec pliku: %s\n", nazwaPlikuIn);
...
int main(int argc, char *argv[]) {
    ...
    // Zapisywanie wyniku CRC na koñcu pliku wejœciowego
    zapiszCRCNaKoncuPliku(argv[1], CRC);
